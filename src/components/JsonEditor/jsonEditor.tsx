...
function JsonEditor({
	...
	const [selectedFieldsForEditing, setSelectedFieldsForEditing] = useState<
		Record<string, any>
	>({});
	...
	if (!deepEqual(json, jsonRef.current)) {
	...
	if (
		!deepEqual(editableFieldsRef.current, editableFields) ||
		!deepEqual(nonEditableFieldsRef.current, nonEditableFields)
	) {
	...
	const handleOnChange: HandleOnChange = (
		value,
		path,
	...
	const handleOnSubmit: HandleOnSubmit = (value, path) => {
		...
		if (onSubmit && jsonState) {
			onSubmit({
				...
				editorMode: editingMode,
				submitType:
					editingMode === GLOBAL_INDIVIDUAL_EDITING_MODE
						? INDIVIDUAL_EDITING_MODE
						: editingMode,
			});
		}
	...
	const renderJson = (
		...
		if (typeof value === "object" && value !== null) {
			...
			return <RenderValue value={value} path={path} />;
		}
	};
	const noOfValidationErrors = Object.values(validations).reduce(
		(count, value) => {
	...
	return (
		...
		>
			<div style={styles} className={cn("w-full h-auto", className)}>
				{renderJson(jsonState)}
				{[GLOBAL_EDITING_MODE, GLOBAL_INDIVIDUAL_EDITING_MODE].includes(
					editingMode,
				) &&
					isEditing &&
